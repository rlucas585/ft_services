...
envsubst '${EXTERNAL_IP} ${WP_USER} ${WP_PASS} ${MYSQL_DB} ${MYSQL_SERVNAME} ${FTP_USER} ${FTP_PASS}' < /tmp/wp-config.php > /www/wp-config.php

wp core is-installed
if [ $? == 1 ]
then
	wp core download
	wp core install --url=$EXTERNAL_IP --path=/www --title="rlucas Wordpress" --admin_user="$WP_USER" --admin_password="$WP_PASS" --admin_email="rlucas@student.codam.nl" --skip-email
	wp term create category Test
	export ID=$(wp user get "$WP_USER" --field=ID)
	wp post list --field=post_title | grep "ft_services post"
	if [ $? == 1 ]
	then
	...
	wp post list --field=post_title | grep "Leeds best"
	if [ $? == 1 ]
	then
