...
curl --cacert ${CACERT} --header "Authorization: Bearer ${TOKEN}" -X GET ${APISERVER}/api/v1/namespaces/$NAMESPACE/services/$1/ 2>/dev/null| jq -r '.status | .loadBalancer | .ingress | .[] | .ip'
